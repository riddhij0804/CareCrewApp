...
const getEmailTransporter = async () => {
    ...
    throw new Error("Email service not configured. Set SENDGRID_API_KEY or GMAIL_USER + GMAIL_PASSWORD");
};
/**
...
exports.sendCaregiverInvite = functions.firestore
    ...
    try {
        ...
        const deepLink = `carecrew://accept-invite/${inviteCode}?email=${encodeURIComponent(caregiver.contact)}&uid=${encodeURIComponent(uid)}`;
        // Build email content
...
exports.acceptInvite = functions.https.onCall(async (data, context) => {
    ...
    if (!inviteCode || !email || !ownerUid || !password) {
        throw new functions.https.HttpsError("invalid-argument", "Missing required fields: inviteCode, email, uid, password");
    }
    try {
        ...
        if (caregiverSnap.empty) {
            throw new functions.https.HttpsError("not-found", "Invitation not found. Please check the link and try again.");
        }
        ...
        if (!caregiverDoc.ref.path.includes(ownerUid)) {
            throw new functions.https.HttpsError("permission-denied", "Invitation does not match the owner.");
        }
        ...
        if (error.code === "auth/email-already-exists") {
            throw new functions.https.HttpsError("already-exists", "This email is already registered. Please sign in instead.");
        }
        ...
        throw error instanceof functions.https.HttpsError
            ? error
            : new functions.https.HttpsError("internal", error.message || "Failed to accept invitation");
    }
});
